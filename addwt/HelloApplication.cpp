  ...
  : WApplication(env)
{
  ...
  root()->addWidget(Wt::cpp14::make_unique<Wt::WText>("What is your name: ")); // show some text

  nameEdit_ = root()->addWidget(Wt::cpp14::make_unique<Wt::WLineEdit>()); // allow text input
  nameEdit_->setFocus();                              // give focus

  auto button = root()->addWidget(Wt::cpp14::make_unique<Wt::WPushButton>("Enter"));
                                                      // create a button
  button->setMargin(100, Wt::Side::Left);                   // add 5 pixels margin
...
void HelloApplication::greet()
{
   ...
   */
  greeting_->setText("Hi, " + nameEdit_->text() + ".");
}
